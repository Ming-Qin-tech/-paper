    ...
    整理开源数据集格式大小，考虑到要下载，所以应穿插到每天中，下载好了就查看，比如上午看两个dataset的结构，下午看两个，晚上看两个，从常用的到不常用的顺序调研。

    主流研究方向：根据印度小哥的工具书分清2015年之前的每个研究方向的成果，最好找到代码，一定要读代码。
    2015 年之后的自己花时间找的时候要学会根据摘要快速筛选
        先搞清楚名词的意义，这个直接维基百科就行了。
        再集中局部时间分section攻破。
    

    业界现状：每天的11：00 左右找个时间点搜个10分钟，下午 4：40 也一样，找到感兴趣的放入收藏夹。



    （今天搞清楚section所需的方法的大概作用）
    （印度小哥的工具书）
    总体时间安排计划：
        dataset 3 天 18 个数据集调研。
        3-4 天 ，尽量控制在三天梳理一个section，要跑通一个此类型的代码（理解了代码才算过关）

        分section大框架：
            梳理完一个section ----> 再看一个review  ---->看下一个section ----> ..... 循环
       
            一个工作时间段（一个上午或下午）： 
                2 个数据集的调研， 10 分钟业界现状，其余section调研。  （注意，切换的时候要快，尽量降低切换时间损耗）
